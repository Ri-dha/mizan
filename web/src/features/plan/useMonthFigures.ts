...
import { liveIncomeSources, liveReceiptsFor, monthIncome, occurrencesFor, type PlannedOccurrence } from "@/db/income"
import { liveBucketsOf, livePlans, planFor } from "@/db/plan"
import type { Bucket, Debt, DebtPayment, Goal, GoalDeposit, IncomeReceipt, IncomeSource, LedgerTransaction, Plan } from "@/db/schema"
...
const EMPTY: MonthView = {
  plan: undefined, buckets: [], sources: [], receipts: [], occurrences: [],
...
export function useMonthView(monthKey: string, startDay: number): MonthView {
  return useLiveQuery(async () => {
    const [plans, sources, receipts, expenses, paidOccurrences, transactions, debts, debtPayments, goals, goalDeposits] = await Promise.all([
      livePlans(), liveIncomeSources(), liveReceiptsFor(monthKey), liveRecurringExpenses(), liveOccurrencesFor(monthKey),
    ...
    return { plan, buckets, sources, receipts, occurrences, income, bills, transactions, debts, debtPayments, goals, goalDeposits, figures }
